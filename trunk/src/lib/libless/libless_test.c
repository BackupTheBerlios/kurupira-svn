...
int main(int argc, char *argv[]) {
	...
	TRY(libless_setup(&env, &parameters, &master_key), goto end);

	TIMING_AFTER();
	COMPUTE_TIMING(libless_setup);

	printf("System parameters and master key generated.\n");

	TIMING_BEFORE();

	TRY(libless_extract(&env, &partial_key, id, 4, master_key, parameters),
			goto end);

	TIMING_AFTER();
	COMPUTE_TIMING(libless_extract);

	printf("Partial private key extracted.\n");

	TIMING_BEFORE();

	TRY(libless_set_secret(&env, &secret, parameters), goto end);

	TIMING_AFTER();
	COMPUTE_TIMING(libless_set_secret);

	printf("Secret value set.\n");

	TIMING_BEFORE();

	TRY(libless_set_public(&env, &public_key, secret, parameters), goto end);

	TIMING_AFTER();
	COMPUTE_TIMING(libless_set_public);

	printf("Public key set.\n");

	TIMING_BEFORE();

	TRY(libless_set_private(&env, &private_key, secret, partial_key,
					parameters), goto end);

	TIMING_AFTER();
	COMPUTE_TIMING(libless_set_private);
