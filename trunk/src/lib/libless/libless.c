 ...
 */
#define POINT_SIZE	(P_SIZE_BYTES + 1)

/**
 * Size in bytes of the pseudo-random generator seed.
 */
#define SEED_SIZE_BYTES	4
...
/**
 * Hashes an arbitrary byte vector. The output buffer must have HASH_SIZE
 * bytes.
 * 
 * @param[in,out] env   - the library context
 * @param[out] out      - the resulting hash value
 * @param[in] in        - the data to be hashed
 * @param[in] in_len    - the length of data in bytes
 * @returns LIBLESS_OK if no error occurs, LIBLESS_ERROR otherwise.
 */

static int hash_function(libless_t *env, unsigned char *out, unsigned char *in,
		int in_len);

/**
 * Hashes an arbitrary byte vector to a integer modulo p. The function computes
 *  iterated hash functions of the byte vector. The hash function used is the
 * SHA1 function, as defined in IBCS#1. A valid OpenSSL context is passed 
 * to the function to optimize memory allocation.
 * 
 * @param[in,out] env   - the library context
 ...
 * @returns LIBLESS_OK if no error occurs, LIBLESS_ERROR otherwise.
 */
static int hash_to_integer(libless_t *env, BIGNUM *number, unsigned char *in,
		int in_len, BIGNUM *p);
...
void libless_init(libless_t *env) {
	env->code = LIBLESS_OK;
	strncpy(env->reason, "", ERROR_LENGTH);

	RAND_load_file("/dev/random", SEED_SIZE_BYTES);
...
int libless_setup(libless_t *env, libless_params_t * parameters,
...
int libless_extract(libless_t *env, libless_partial_t * key, unsigned char *id,
	...
	TRY(hash_to_integer(env, h, id, id_len, parameters.factor),
			ERR(REASON_HASH));
...
int libless_set_public(libless_t *env, libless_public_t * public_key,
	...
	TRY(*public_key = BN_new(), ERR(REASON_MEMORY));

	/* The public key is a power of a pairing. So, we have to compute this power
	 * using the special function. */
	TRY(libless_pairing_power(env, *public_key, parameters.pairing, secret,
...
int libless_set_private(libless_t *env, libless_private_t * private_key,
...
int libless_sign(libless_t *env, libless_signature_t * signature,
	...
	TRY(image_bin = (unsigned char *)
			calloc(POINT_SIZE, sizeof(unsigned char)), ERR(REASON_MEMORY));
	...
	TRY(BN_bn2bin(public_key, h_bin + in_len + id_len), ERR(REASON_OPENSSL));
	TRY(BN_bn2bin(r, h_bin + in_len + id_len + P_SIZE_BYTES),
			ERR(REASON_OPENSSL));
	TRY(hash_to_integer(env, h, h_bin, h_len, parameters.factor),
			ERR(REASON_HASH));
	...
	TRY(EC_POINT_point2oct(parameters.group1, image,
					POINT_CONVERSION_COMPRESSED, image_bin, POINT_SIZE, ctx),
			ERR(REASON_OPENSSL));

	signature->hash = hash_bin;
	signature->image = image_bin;
	signature->image_len = POINT_SIZE;
...
int libless_verify(libless_t *env, int *verified,
	...
	TRY(EC_POINT_oct2point(parameters.group1, image, signature.image,
					signature.image_len, ctx), ERROR(REASON_OPENSSL, 10));
	...
	TRY(hash_to_integer(env, h1, id, id_len, parameters.factor),
			ERR(REASON_HASH));
	...
	TRY(libless_pairing_power(env, r, public_key, hash, parameters, ctx),
			ERR(REASON_PAIRING));
	...
	TRY(BN_bn2bin(r1, h_bin + in_len + id_len + P_SIZE_BYTES),
			ERR(REASON_OPENSSL));
	TRY(hash_to_integer(env, h1, h_bin, h_len, parameters.factor),
			ERR(REASON_HASH));

	memset(h_bin + in_len + id_len + P_SIZE_BYTES, 0, P_SIZE_BYTES);
	TRY(BN_bn2bin(r2, h_bin + in_len + id_len + P_SIZE_BYTES),
			ERR(REASON_OPENSSL));
	TRY(hash_to_integer(env, h2, h_bin, h_len, parameters.factor),
			ERR(REASON_HASH));
...
static int hash_function(libless_t *env, unsigned char *out,
...
static int hash_to_integer(libless_t *env, BIGNUM *number, unsigned char *in,
	...
	TRY(h = BN_CTX_get(ctx), ERR(REASON_MEMORY));
	TRY(h_bin = (unsigned char *)calloc((HASH_SIZE + in_len),
	...
	memcpy(h_bin + HASH_SIZE, in, in_len);
	h_len = HASH_SIZE + in_len;

	/* Iterate the hash function. */
	for (i = 0; i < limit; i++) {
		memcpy(h_bin, digest, HASH_SIZE);
		TRY(hash_function(env, digest, h_bin, h_len), ERR(REASON_HASH));
		/* Shift the current number, convert the hash into another number and
		 * add them. */
		TRY(BN_lshift(number, number, HASH_SIZE << 3), ERR(REASON_OPENSSL));
		TRY(BN_bin2bn(digest, HASH_SIZE, h), ERR(REASON_OPENSSL));
